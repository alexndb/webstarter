...
$admin_email  = "noz2008@gmail.com"; // your mail for messages
...
foreach ( $_POST as $key => $value ) {
	if ( $value != "" && $key != "project_name" && $key != "admin_email" ) {
		...
		switch ($key) {
			case 'form_name': $key = "Идентификатор фомы"; continue;
			case 'name': $key = "Имя"; continue;
			case 'email': $key = "Email"; continue;
			case 'phone': $key = "Телефон"; continue;
			case 'text': $key = "Сообщение"; continue;
			case 'select': $key = "Пункт списка"; continue;
		}
		// message
		$message .= "" . ( ($c = !$c) ? '<tr>':'
...
// send mail
mail($admin_email, adopt($mail_title), $message, $headers );